...
def show_cam_on_image(img, mask, path):
    ...
    cam = cam / np.max(cam)
    cv2.imwrite(path + "_cam.jpg", np.uint8(255 * cam))
...
if __name__ == "__main__":
    ...
    model = model.module.cnn
    grad_cam = GradCam(model=model, feature_module=model[7], target_layer_names=["2"], use_cuda=True)
    ...
    show_cam_on_image(img, mask, args.image_path.split('/')[-1])
