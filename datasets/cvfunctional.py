...
# from datasets.dct_resize.dct import dct2, idct2
from sklearn.preprocessing import minmax_scale
from jpeg2dct.numpy import load, loads
# from datasets.dct_resize.resize_dct_opencv import resize_dct
import skimage
# from datasets.dct_resize.resize_dct_opencv import blockshaped, unblockshaped
...
def dct_unflatten_3d(img, N=8):
    height_resized, width_resized = img.shape
    img = blockshaped(img, N, N)
    num_blk = img.shape[0]
    height_reform, width_reform = height_resized // N, width_resized // N
    img = img.reshape((num_blk, -1))
    img = img.reshape((height_reform, width_reform, N ** 2))
    return img
...
def resize_dct_tensor(img, size, A1=None, A2=None):
    """Resize the input CV Image to the given size.
    ...
    if isinstance(size, int):
        ...
        if w < h:
            ow = size
            oh = int(size * h / w)
            img = resize_dct(img, L=1, M=1, T=(oh, ow), A1=A1, A2=A2, debug=False, new=True)
            return img
            # return cv2.resize(img, dsize=(ow, oh), interpolation=INTER_MODE[interpolation])
        else:
            oh = size
        ...
        oh, ow = size
        img, _, _ = resize_dct(img, L=1, M=1, T=(oh, ow), A1=A1, A2=A2, debug=False, new=True)
        return img

        # debug code
        # img, rgb_y, y_raw = resize_dct(img, L=1, M=1, T=(oh, ow), A1=A1, A2=A2, debug=True, new=True)
        # dct_reform_show = minmax_scale(img[:, :, 0])
...
def resized_crop_dct(img, i, j, h, w, size, A1=None, A2=None):
    """Crop the given CV Image and resize it to desired size. Notably used in RandomResizedCrop.
    ...
    img = crop(img, i, j, h, w)
    # cv2.imwrite('/mnt/ssd/kai.x/work/code/iftc/datasets/cvtransforms/test/crop.jpg', img)
    img = resize_dct_tensor(img, size, A1, A2)
    # cv2.imwrite('/mnt/ssd/kai.x/work/code/iftc/datasets/cvtransforms/test/resize.jpg', img)
    return img
