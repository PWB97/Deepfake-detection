...
resnet = Baseline(**config.net_params)
device_count = torch.cuda.device_count()
print('使用4个GPU')
model = nn.DataParallel(resnet)
resnet.to(device)
ckpt = torch.load('/data2/guesthome/wenbop/modules/ff/bi-model_type-baseline_gru_ep-19.pth')
...
class ModelOutputs():
    ...
    def __call__(self, x):
        ...
        if self.cuda:
            ...
            cnn = []
            cnn.append(output)
            cnn = torch.stack(cnn, dim=0).transpose(0, 1)
            rnn_out, _  = resnet.rnn(cnn)
            output = resnet.fc_cnn(rnn_out[:,0,:]).cuda()##这里就是为什么我们多加载一个resnet模型进来的原因，因为后面我们命名的model不包含fc层，但是这里又偏偏要使用。#
        else:
            cnn = []
            cnn.append(output)
            cnn = torch.stack(cnn, dim=0).transpose(0, 1)
            rnn_out, _  = resnet.rnn(cnn)
            output = resnet.fc_cnn(rnn_out[:,0,:])##这里对应use-cuda上更正一些bug,不然用use-cuda的时候会导致类型对不上,这样保证既可以在cpu上运行,gpu上运行也不会出问题.
...
def get_args():
    ...
    parser.add_argument('--image-path', type=str, default='/data2/guesthome/wenbop/ffdf/test/0/',
