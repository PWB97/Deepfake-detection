...
class ResNet50DCT(nn.Module):
    def __init__(self):
        ...
        self.input_layer = nn.Sequential(
            nn.Conv2d(192, 64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
...
def main():
    ...
    y  = torch.from_numpy(np.random.randn(16, 64, 112, 112)).float()
    cb = torch.from_numpy(np.random.randn(16, 64, 112, 112)).float()
    cr = torch.from_numpy(np.random.randn(16, 64, 112, 112)).float()
    x = model_resnet50dct(y, cb, cr)
    print(x.shape)
